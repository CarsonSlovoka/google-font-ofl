/**
 * Google Fonts OFL upstream GitHub browser — v2
 * data from enrich_github_data_tree.py
 * + sources/*.glyphs / sources/*.glyphspackage filters
 */
...
(function () {
  ...
  const state = {
    ...
    onlyGlyphs: false,
    onlyGlyphspackage: false,
    onlyAnyGlyphsSource: false,
    meta: { success: 0, fail: 0, generated_at: "", enrich_version: "" },
  };
  ...
  const hasGlyphsEl = $("#has-glyphs");
  const hasPkgEl = $("#has-glyphspackage");
  const hasAnyGlyphsEl = $("#has-any-glyphs-source");
  ...
  function applyFilterAndSort() {
    ...
    if (state.onlyGlyphs) {
      list = list.filter((it) => it.has_glyphs === true);
    }

    if (state.onlyGlyphspackage) {
      list = list.filter((it) => it.has_glyphspackage === true);
    }

    if (state.onlyAnyGlyphsSource) {
      list = list.filter((it) => it.has_glyphs === true || it.has_glyphspackage === true);
    }
  ...
  function renderSourceBadges(item) {
    const bits = [];
    if (item.has_glyphs) {
      bits.push(`<span class="source-badge glyphs" title="sources/*.glyphs">.glyphs</span>`);
    }
    if (item.has_glyphspackage) {
      bits.push(
        `<span class="source-badge pkg" title="sources/*.glyphspackage">.glyphspackage</span>`
      );
    }
    return bits.length ? `<div class="card-meta-row">${bits.join("")}</div>` : "";
  }
  ...
  function renderCard(item) {
    ...
    if (item.tree_method && item.tree_method !== "none") {
      parts.push(
        `<span class="pill" title="enrich method">${escapeHtml(item.tree_method)}</span>`
      );
    }
    meta.innerHTML = parts.join("") || `<span class="pill">點卡片看 GitHub 統計</span>`;

    const sourceBadges = document.createElement("div");
    sourceBadges.innerHTML = renderSourceBadges(item);

    const filesBlock = document.createElement("div");
    filesBlock.className = "card-files";
    filesBlock.innerHTML =
      renderFileLinks(item.glyphs_sources, "sources/*.glyphs") +
      renderFileLinks(item.glyphspackage_sources, "sources/*.glyphspackage") +
    ...
    if (sourceBadges.innerHTML.trim()) card.appendChild(sourceBadges.firstChild);
  ...
  function updateBadges() {
    ...
    const ev = state.meta.enrich_version || "—";
    $("#badge-enrich").textContent = `enrich：${ev}`;
  }
  ...
  async function loadData() {
    try {
      ...
      for (const it of state.items) {
        ...
        if (!Array.isArray(it.glyphs_sources)) it.glyphs_sources = [];
        if (!Array.isArray(it.glyphspackage_sources)) it.glyphspackage_sources = [];
        if (typeof it.has_glyphs !== "boolean") {
          it.has_glyphs = it.glyphs_sources.length > 0;
        }
        if (typeof it.has_glyphspackage !== "boolean") {
          it.has_glyphspackage = it.glyphspackage_sources.length > 0;
        }
      }
      state.meta = {
        ...
        enrich_version: data.enrich_version || "tree-v1?",
      };
      updateBadges();
  ...
  hasGlyphsEl.addEventListener("change", () => {
    state.onlyGlyphs = hasGlyphsEl.checked;
    applyFilterAndSort();
  });

  hasPkgEl.addEventListener("change", () => {
    state.onlyGlyphspackage = hasPkgEl.checked;
    applyFilterAndSort();
  });

  hasAnyGlyphsEl.addEventListener("change", () => {
    state.onlyAnyGlyphsSource = hasAnyGlyphsEl.checked;
    applyFilterAndSort();
  });
